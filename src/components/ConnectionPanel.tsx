...
export function ConnectionPanel({ pairingCode, qrValue, invitationText, joinCode, onJoinCodeChange, onJoin, onRefresh, busy }: Props) {
  ...
  const copyInvitation = async () => { if (navigator.clipboard) { await navigator.clipboard.writeText(invitationText); } else { const input = document.createElement('textarea'); input.value = invitationText; document.body.append(input); input.select(); document.execCommand('copy'); input.remove(); } setCopied(true); };
