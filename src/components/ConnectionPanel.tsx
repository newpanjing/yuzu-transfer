...
export function ConnectionPanel({ pairingCode, qrValue, invitationText, joinCode, onJoinCodeChange, onJoin, onRefresh, busy }: Props) {
  ...
  return <section className="connection-panel"><div className="connection-section"><span className="connection-icon"><MonitorSmartphone size={21} /></span><div className="connection-copy"><h2>{t('connect.myCode.title')}</h2></div><VerificationCodeDisplay code={pairingCode} /><div className="connection-actions"><button className="text-button" onClick={() => void copyInvitation()}>{copied ? <Check size={14} /> : <Copy size={14} />}{copied ? t('connect.copied') : t('connect.copy')}</button><button className="text-button refresh-button" onClick={onRefresh}><RefreshCw size={14} /> {t('connect.refresh')}</button></div></div><div className="connection-divider" /><div className="connection-section qr-section"><span className="connection-icon"><QrCode size={21} /></span><div className="connection-copy"><h2>{t('connect.qr.title')}</h2></div><div className="qr-frame">{qrImage ? <img className="qr-image" src={qrImage} alt={t('connect.qr.alt')} /> : <div className="qr-placeholder" />}<div ref={qrHost} className="qr-canvas-host" aria-hidden="true"><QRCodeCanvas value={qrValue} size={QR_CODE_SIZE} bgColor="#ffffff" fgColor="#161b20" level="M" includeMargin /></div></div></div><div className="connection-divider" /><div className="connection-section join-section"><span className="connection-icon"><Smartphone size={21} /></span><div className="connection-copy"><h2>{t('connect.join.title')}</h2></div><PairingCode value={joinCode} onChange={onJoinCodeChange} /><button className="primary-button" disabled={joinCode.length !== 4 || busy} onClick={onJoin}>{busy ? t('connect.join.busy') : t('connect.join.action')}</button></div></section>;
}
