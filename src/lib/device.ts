...
export function getNickname() { const saved = sessionStorage.getItem(STORAGE_KEYS.nickname); if (saved) return saved; const language = detectSystemLanguage(); const names = DEVICE_NAMES[language]; const name = `${names[Math.floor(Math.random() * names.length)]}${Math.floor(100 + Math.random() * 900)}`; sessionStorage.setItem(STORAGE_KEYS.nickname, name); return name; }
