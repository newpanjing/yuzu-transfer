...
export default function App() {
  const [view, setView] = useState<View>('connect'); const [deviceId] = useState(getDeviceId); const [nickname, setNickname] = useState(getNickname); const [pairingCode, setPairingCode] = useState(''); const [joinCode, setJoinCode] = useState(''); const [busy, setBusy] = useState(false); const [error, setError] = useState(''); const [relayLimit, setRelayLimit] = useState(DEFAULT_RELAY_LIMIT);
  ...
  useEffect(() => { const code = new URLSearchParams(location.search).get('code'); if (code) setJoinCode(code); }, []);
  const join = async () => { setBusy(true); setError(''); try { await exchangePairing(joinCode, deviceId); setView('transfer'); } catch { setError('验证码无效、已过期，或不能连接当前设备。'); } finally { setBusy(false); } };
  const changeNickname = (name: string) => { saveNickname(name); setNickname(name); };
  const qrValue = `${location.origin}?code=${pairingCode}`;
  return <div className="app-shell"><header><Brand /><div className="header-meta"><ShieldCheck size={17} /> 不登录 · 不存文件 · 不留记录</div></header><div className="app-body"><Sidebar active={view} onSelect={setView} />{view === 'connect' ? <main className="connect-view"><div className="view-heading"><span className="eyebrow"><BadgeCheck size={17} /> 安全直连</span><h1>连接新设备</h1><p>在手机上打开柚子快传，选择以下任一方式连接</p></div><div className="pairing-grid"><JoinCard code={joinCode} onChange={setJoinCode} onJoin={() => void join()} busy={busy} /><QrCard value={qrValue} /><PairingCard code={pairingCode} onRefresh={() => void refresh()} /></div>{error && <div className="error-banner">{error}</div>}<div className="privacy-banner"><ShieldCheck size={20} /><span><strong>无痕传输</strong> · 文件优先在设备间直连，不保存至服务器</span></div></main> : <TransferWorkspace nickname={nickname} onNicknameChange={changeNickname} relayLimit={relayLimit} />}</div><footer><span>设备 ID：{deviceId.slice(0, 8)}</span><span><Monitor size={15} /> 当前设备已就绪</span><span><ShieldCheck size={16} /> 无痕模式已开启</span></footer></div>;
}
